...
/* carrello con gli acquisti. Vettore di oggetti contenenti codice, prezzo, quantita
                                       di ciascun prodotto */
...
function serializza() {
    /* serializza il carrello in oggetti e lo memorizza mediante cookies (localStorage)
...
function eliminaElementoDalCarrello() {
    /*elimina il prodotto dalla tabella quando la quantità è minore di 1 */
    for (var i = 0; i < carrello.length; i++) {
        if (carrello[i].qnt < 1 || carrello[i] === "") {
            var numeroElemento = carrello[i];
        }
        if (carrello[i] === numeroElemento) {
            carrello.splice(i, 1);
        }
    }
...
function tabella() {
    ...
    document.write("<TABLE id='table' border=1><TH>Cod<TH>Prodotto<TH>Prezzo<TH>Quantita'<TH>Tot\n ");
    for (var i = 0; i < carrello.length; i++) {
        document.write("<TR><TD class=center>" + carrello[i].codice);
        document.write("<TD id='desc'> " + carrello[i].descr);
        document.write("<TD class=right>" + carrello[i].prezzo);
        document.write("<TD id='costo'><input onChange=cambia(" + i + ") class=center id=q" + i + " type=text size=4 value= " + carrello[i].qnt + ">");
